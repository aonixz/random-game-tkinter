...
def check_number(one, two, three, four, five, gen, root, start, Id,page):
    start_list = list()
    stop_list = list()
    if len(one.get()) == 1 and len(two.get()) == 1 and len(three.get()) == 1 and len(four.get()) == 1 and len(five.get()) == 1:
        c = count()
        for j in start.split(":"):
            start_list.append(int(j))
        answer = [one.get(), two.get(), three.get(), four.get(), five.get()]
        if answer[0] == gen[0]:
        ...
        if answer[::] == gen[::]:
            ...
            save_data(Id, level="level 5", time=timer, generate_number=string_number(gen), count=len(c))
            root.destroy()
            page(Id)
    else:
        messagebox.showerror("Error", 'The number entered must be a single digit')

def level_five(Id,page):
    level_Five = Tk()
    level_Five.title('Level Five')
    one_num = StringVar()
    two_num = StringVar()
    three_num = StringVar()
    four_num = StringVar()
    ...
    Entry(level_Five, textvariable=one_num, width=5,justify=CENTER).grid(row=0, column=0, padx=5, ipady=10)
    Entry(level_Five, textvariable=two_num, width=5,justify=CENTER).grid(row=0, column=1, padx=5, ipady=10)
    Entry(level_Five, textvariable=three_num, width=5,justify=CENTER).grid(row=0, column=2, padx=5, ipady=10)
    Entry(level_Five, textvariable=four_num, width=5,justify=CENTER).grid(row=0, column=3, padx=5, ipady=10)
    Entry(level_Five, textvariable=five_num, width=5,justify=CENTER).grid(row=0, column=4, padx=5, ipady=10)
    start = time.strftime(" %H:%M:%S ", time.gmtime())
    gen = generate_number(5)
    Button(level_Five, text='Check',width=30,
           command=lambda: check_number(one_num, two_num, three_num, four_num, five_num, gen, level_Five, start, Id,page)).grid(row=1,
                                                                                                                 column=1, columnspan=3,pady=5)
